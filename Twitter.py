...
def analyze_twitter_comments():
    data = request.get_json()
    comments = data.get('comments', [])
    
    print(f"🐦 Nœud Twitter: traitement de {len(comments)} commentaires...")
    
    result = map_sentiment_analysis(comments)
    
    print(f"✅ Nœud Twitter: analyse terminée - {result['comments_processed']} commentaires traités")
    
    return jsonify(result)

@app.route('/health', methods=['GET'])
def health_check():
    return jsonify({'status': 'healthy', 'node': 'twitter_processor'})

if __name__ == '__main__':   
    print("🐦 Nœud Twitter d'Analyse de Sentiments démarré")
    print("📍 Port: 5001")
    print("📊 Responsable: Traitement des commentaires Twitter")
    print("🌐 Accessible depuis: http://<IP_MACHINE2>:5001")
